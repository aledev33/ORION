...
_DEVICE_RATE = 44100
_VOSK_RATE   = 16000
# Bloque más pequeño = menor latencia (~80ms en lugar de 125ms)
_BLOCKSIZE   = int(_DEVICE_RATE * 0.08)
...
class Listener:
    ...
    def _callback(self, indata, frames, time_info, status):
        try:
            ...
            resampled   = soxr.resample(audio_f32, _DEVICE_RATE, _VOSK_RATE, quality='QQ')
    ...
    def _escuchar_con_recognizer(self, recognizer, segundos_max: int,
                                  max_silencio: int = 6) -> str:
        self._clear_queue()
        partes: list[str] = []
        bloques_max      = int(segundos_max / 0.08) + 1
        bloques_silencio = 0
    ...
    def escuchar_hotword_y_comando(self, segundos_max: int = 8) -> tuple[bool, str]:
        ...
        texto = self._escuchar_con_recognizer(
            recognizer, segundos_max=segundos_max, max_silencio=5
        )
        ...
        comando = texto_lower
        for hw in _HOTWORDS:
            comando = re.sub(r"^" + hw + r"[,\s]*", "", comando, flags=re.IGNORECASE).strip()
